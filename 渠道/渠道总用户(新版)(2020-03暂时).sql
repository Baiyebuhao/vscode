---渠道总用户(新版)(2020-03暂时)(2020-02-06更新)
SELECT data_source,
       the_2nd_level,
       the_3rd_level,
       chan_no,
...
where a.registe_date between '2020-03-01' and '2020-03-31'                  ---------选择注册时间维度
...
where substring(a.registe_date,1,10) between '2020-03-01' and '2020-03-31'                  ---------选择注册时间维度
...
where a.data_source not in ('jry','bhd')
  and c.apply_time between '2020-03-01' and '2020-03-31'                ---------选择申请时间维度
...
from warehouse_atomic_user_info a   
     ...
     left join warehouse_data_user_review_info c
        ...
        and a.data_source = c.data_source
where a.data_source not in ('jry','bhd')
  and c.credit_time between '2020-03-01' and '2020-03-31'                ---------选择授信时间维度
  and c.status = '通过'
...
from warehouse_atomic_user_info a   
     ...
     left join warehouse_data_user_withdrawals_info c
        ...
        and a.data_source = c.data_source
where a.data_source not in ('jry','bhd')
  and c.cash_time between '2020-03-01' and '2020-03-31'                ---------选择提现时间维度
  and c.cash_amount > 0
