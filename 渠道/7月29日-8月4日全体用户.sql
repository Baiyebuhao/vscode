...
where a.registe_date between '2019-07-29' and '2019-08-04'                  ---------选择注册时间维度
UNION ALL
--------用户实名
select '实名' AS code,
       a.data_source, 
...
from warehouse_atomic_user_info a   
 ...
 where c.action_name = '实名'
   and c.action_date between '2019-07-29' and '2019-08-04'                  ------------选择实名时间维度
UNION ALL
--------用户申请
 select '申请' AS code,
       a.data_source,
...
from warehouse_atomic_user_info a   
     ...
     left join warehouse_data_user_review_info c
         on a.mbl_no = c.mbl_no 
        and a.data_source = c.data_source
where c.apply_time between '2019-07-29' and '2019-08-04'                ---------选择申请时间维度
UNION ALL
--------用户授信
 select '授信' AS code,
       a.data_source,
...
from warehouse_atomic_user_info a   
     ...
     left join warehouse_data_user_review_info c
         on a.mbl_no = c.mbl_no 
        and a.data_source = c.data_source
where c.credit_time between '2019-07-29' and '2019-08-04'                ---------选择授信时间维度
  and c.status = '通过'
UNION ALL
----------用户提现
select '提现' AS code,
...
from warehouse_atomic_user_info a   
     ...
     left join warehouse_data_user_withdrawals_info c
         on a.mbl_no = c.mbl_no 
        and a.data_source = c.data_source
where c.cash_time between '2019-07-29' and '2019-08-04'                ---------选择提现时间维度
