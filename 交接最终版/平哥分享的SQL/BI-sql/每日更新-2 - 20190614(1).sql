...
FROM
  (SELECT a.data_source,
   ...
   UNION ALL SELECT d.data_source,
                    ...
                    sum(CASE
                            WHEN product_name = '随借随还-马上' THEN cashamount*pre
                        END) AS cash01,
                    sum(CASE
                            WHEN product_name = '随借随还-众安' THEN cashamount*pre
                        END) AS cash02,
                    sum(CASE
                            WHEN product_name = '随借随还-钱包易贷' THEN cashamount*pre
                        END) AS cash03,
                    sum(CASE
                            WHEN product_name = '现金分期-马上' THEN cashamount*pre
                        END) AS cash04,
                    sum(CASE
                            WHEN product_name = '现金分期-中邮' THEN cashamount*pre
                        END) AS cash05,
                    sum(CASE
                            WHEN product_name = '现金分期-万达普惠' THEN cashamount*pre
                        END) AS cash06,
                    sum(CASE
                            WHEN product_name = '现金分期-点点' THEN cashamount*pre
                        END) AS cash07,
                    sum(CASE
                            WHEN product_name = '现金分期-兴业消费' THEN cashamount*pre
                        END) AS cash08,
                    sum(CASE
                            WHEN product_name = '现金分期-小雨点' THEN cashamount*pre
                        END) AS cash09,
                    sum(CASE
                            WHEN product_name = '现金分期-拉卡拉' THEN cashamount*pre
                        END) AS cash10,
                    sum(CASE
                            WHEN product_name = '大期贷-招联' THEN cashamount*pre
                        END) AS cash11,
                    sum(CASE
                            WHEN product_name = '白领贷-招联' THEN cashamount*pre
                        END) AS cash12,
                    sum(CASE
                            WHEN product_name = '好期贷-招联' THEN cashamount*pre
                        END) AS cash13,
                    sum(CASE
                            WHEN product_name = '业主贷-招联' THEN cashamount*pre
                        END) AS cash14,
                    sum(CASE
                            WHEN product_name = '信用钱包' THEN cashamount*pre
                        END) AS cash15,
                    sum(CASE
                            WHEN product_name = '现金分期-钱伴' THEN cashamount*pre
                        END) AS cash16,
                    sum(CASE
                            WHEN product_name = '现金分期-招联' THEN cashamount*pre
                        END) AS cash17
...
FROM
  (SELECT '注册' AS TYPE,
          ...
          if(child_chan IS NULL,'空白',child_chan) AS the_4th_level,
   ...
   GROUP BY data_source,
            ...
            if(child_chan IS NULL,'空白',child_chan)
   ...
   GROUP BY a.data_source,
            ...
            if(b.the_2nd_level IS NULL,'空白',b.the_2nd_level),
            if(b.the_3rd_level IS NULL,'空白',b.the_3rd_level),
            if(child_chan IS NULL,'空白',child_chan)) AS a
...
-- 三平台用户转化明细数据(周报分析) add 2019-06-14
