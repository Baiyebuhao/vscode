...
where a.action_name = '注册'and a.data_source <> 'jry'
      and a.action_date between '2019-02-20' and '2019-02-26'                  ---------选择注册时间维度
...
where a.status_desc in('自认证','认证成功') 
  and a.action_date between '2019-02-20' and '2019-02-26'                ---------选择实名时间维度
)a2                                                                       ---------取出实名用户作为表a2
...
where a.mbl_no <> 'null'
	 and a.apply_time between '2019-02-20' and '2019-02-26'           ---------选择申请时间维度
...
where a.mbl_no <> 'null'
     and a.status = '通过'
	 and a.credit_time between '2019-02-20' and '2019-02-26'          ---------选择授信时间维度
     and a1.action_date <> 'null'
group by substring(a1.action_date,1,7),a1.data_source,a1.2nd_level,a1.chan_no_desc,a1.child_chan
)                                                                     ----------授信汇总表
...
where a.mbl_no <> 'null'
    and a.cash_amount > 0
	and a.cash_time between '2019-02-20' and '2019-02-26'                       ---------选择提现时间维度 
    and a1.action_date <> 'null'
group by substring(a1.action_date,1,7),a1.data_source,a1.2nd_level,a1.chan_no_desc,a1.child_chan
)                                                                               ---------提现汇总表 
