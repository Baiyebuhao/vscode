...
select '实名' AS code,
	   ...
	   0 as cash_amount
from warehouse_atomic_user_info a   
  join warehouse_data_chan_info b 
    on a.data_source = b.data_source 
   and a.chan_no = b.chan_no 
   and a.child_chan = b.child_chan
 ...
 where a.registe_date between '2019-03-01' and '2019-03-31'    
   ...
   and c.action_date between '2019-03-01' and '2019-03-31'               
 )
( --------新注册用户申请
 select '申请' AS code,
       a.data_source,
...
from warehouse_atomic_user_info a   
     ...
     join warehouse_data_user_review_info c
        ...
        and a.data_source = c.data_source
where a.registe_date between '2019-03-01' and '2019-03-31'              ---------选择注册时间维度
  and c.apply_time between '2019-03-01' and '2019-03-31'                ---------选择申请时间维度
 )
( --------新注册用户授信
 select '授信' AS code,
       a.data_source,
...
from warehouse_atomic_user_info a   
     ...
     join warehouse_data_user_review_info c
        ...
        and a.data_source = c.data_source
where a.registe_date between '2019-03-01' and '2019-03-31'              ---------选择注册时间维度
  and c.credit_time between '2019-03-01' and '2019-03-31'                ---------选择授信时间维度
  and c.status = '通过'
 )
( ----------新注册用户提现
select '提现' AS code,
...
from warehouse_atomic_user_info a   
     join warehouse_data_chan_info b 
        ...
        and a.chan_no = b.chan_no 
        and a.child_chan = b.child_chan        
     join warehouse_data_user_withdrawals_info c
         on a.mbl_no = c.mbl_no 
        and a.data_source = c.data_source
where a.registe_date between '2019-03-01' and '2019-03-31'              ---------选择注册时间维度
  and c.cash_time between '2019-03-01' and '2019-03-31'                ---------选择提现时间维度
  and c.cash_amount > 0
)