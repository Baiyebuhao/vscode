--2.申请
select substr(a.loan_apply_time,1,7) as extractday,
       a.bank_name,
	   --a.dot_id,                                                --网点ID
	   --a.dot_name,
...
--调查
select substr(a.research_apply_time,1,7) as extractday,
       a.bank_name,
	   --a.dot_id,                                                --网点ID
	   --a.dot_name,
...
select --substr(a.loan_apply_time,1,10) as extractday,
	   ...
	   --0 as c_mount,                        --建档ID数
	   --0 as mbl_mount,                      --建档号码数
	   count(a.mobile) as app_mount,                   --申请人数
	   count(case when a.m_state = '5' then a.mobile end) as app_over_mount,  --申请完成人数
	   count(case when a.m_state = '6' then a.mobile end) as app_over_mount2  --无申请模式营销完成
	   --0 as research_mount,              --调查人数
	   --0 as research_over_mount,  --调查完成人数
	   --0 as research_wwc_mount,    --调查未完成人数
	   --0 as research_tg_mount,  --调查通过人数
...
select --substr(a.research_apply_time,1,10) as extractday,
	   ...
	   --0 as app_over_mount,                                     --申请完成人数
	   --0 as app_over_mount2,                                    --无申请模式营销完成
	   count(a.mobile) as research_mount,              --调查人数
	   count(case when a.research_status in ('4','5') then a.mobile end) as research_over_mount,  --调查完成人数
	   count(case when a.research_status in ('-1','1','2','3') then a.mobile end) as research_wwc_mount,    --调查未完成人数
	   count(case when a.research_status = '4' and rec_amount > '0' then a.mobile end) as research_tg_mount,  --调查通过人数（调查完成4，是否出额？后台？）
	   count(case when a.research_status = '5' then a.mobile end) as research_jj_mount,    --调查拒绝人数
	   sum(case when a.research_status = '4' and rec_amount > '0' and rec_amount is not NULL then rec_amount end) as res_amount   --调查通过金额
from
(SELECT a.bank_id AS bank_id,                                     --银行ID
       c.name AS bank_name,                                      --银行名称
...
WHERE substr(a.loan_apply_time,1,10) >= '2019-09-01'
  and substr(a.research_apply_time,1,10) between '2020-03-01' and '2020-03-31'
  AND c.name = '秦皇岛抚宁家银村镇银行'
  AND d.name NOT LIKE '%测试%') a
left join warehouse_atomic_hzx_b_bank_user a1
       on a.a_user_id = a1.id
...
select --substr(a.research_apply_time,1,10) as extractday,
	   ...
	   count(a.mobile) as research_mount,              --调查人数
	   count(case when a.research_status in ('4','5') then a.mobile end) as research_over_mount,  --调查完成人数
	   count(case when a.research_status in ('4','5') and a.second_dValue <= 5 then a.mobile end) as research_over_mount1,
	   count(case when a.research_status in ('4','5') and a.second_dValue <= 15 then a.mobile end) as research_over_mount2,
	   count(case when a.research_status in ('4','5') and a.second_dValue > 15 then a.mobile end) as research_over_mount3,
	   count(case when a.research_status in ('-1','1','2','3') then a.mobile end) as research_wwc_mount,    --调查未完成人数
	   count(case when a.research_status = '4' and rec_amount > '0' then a.mobile end) as research_tg_mount,  --调查通过人数（调查完成4，是否出额？后台？）
	   count(case when a.research_status = '5' then a.mobile end) as research_jj_mount,    --调查拒绝人数
	   sum(case when a.research_status = '4' and rec_amount > '0' and rec_amount is not NULL then rec_amount end) as res_amount   --调查通过金额
...
(SELECT a.bank_id AS bank_id,                                     --银行ID
       ...
       a.research_apply_time AS research_apply_time,		     --调查开始时间
       a.research_over_time AS research_over_time,               --调查完成时间
	   (unix_timestamp(substr(a.research_over_time,1,19))-unix_timestamp(substr(a.research_apply_time,1,19)))/60 as second_dValue,  --间隔分钟数
       a.research_apply_id AS research_apply_id,                 --调查ID
       a.research_status AS research_status,                     --'未调查-1,待预约1,待调查2,调查中3,调查完成4,拒绝本次申请5'
	   a.rec_amount,                                             --调查额度
       d.mobile AS mobile,                                       --建档客户号码
...
--排除bhd用户（排除当月还是所有？？）
left outer join (select * from default.warehouse_atomic_time_user a
           WHERE a.data_source in ('bhh','bhd')
		   AND substr(a.registe_date,1,10) >= '2019-09-27') a5
		   on a1.mobile = a5.mbl_no
where a1.id is not null
  and a1.mobile is not null
  and a1.m_id <> '-1'
  and substr(a1.c_time,1,10) between '2020-03-01' and '2020-03-31'
  and a2.name = '秦皇岛抚宁家银村镇银行'
  and a5.mbl_no is null) a
group by bank_name,
		 --dot_id,
		 --dot_name,
		 user_name
--合并bhd用户
union all
select '秦皇岛抚宁家银村镇银行' as bank_name,
	   '享宇线上' as user_name,                                    --建档客户经理
	   '线上' as data_source,                        --用户来源
	   count(mbl_no) as c_mount,                --建档ID数
	   count(distinct mbl_no) as mbl_mount    --建档号码数
