...
--调查时长（分钟）（秒/60）
--调查时长
select research_apply_id,
       substr(research_over_time,1,19),
       substr(a1.research_apply_time,1,19),
       (hour((substr(a1.research_over_time,1,19)))-hour(substr(research_apply_time,1,19))+(datediff((substr(a1.research_over_time,1,19)), substr(research_apply_time,1,19)))*24) as hour_dValue,
       (unix_timestamp(substr(a1.research_over_time,1,19))-unix_timestamp(substr(research_apply_time,1,19)))/60 as second_dValue
from warehouse_atomic_hzx_research_task a1
left join warehouse_atomic_hzx_b_bank_base_info a2
on a1.bank_id = a2.id
where a1.m_state='5'--营销完成
  and a1.distribution = '1'  --已分配
  and a1.research_status in ('4','5')--待调查2,调查中3,调查完成4,拒绝本次申请5'
  and substr(a1.loan_apply_time,1,10) between '2020-03-01' and '2020-03-31'
  and a2.name = '秦皇岛抚宁家银村镇银行'  